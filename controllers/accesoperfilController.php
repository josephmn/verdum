...
class accesoperfilController extends Controller
{
	...
	public function index()
	{
		if (isset($_SESSION['usuario'])) {
			...
			$result = $soap->ListadoPerfil();
			$perfiles = json_decode($result->ListadoPerfilResult, true);
	...
	public function validar_perfil()
	{
		if (isset($_SESSION['usuario'])) {
			...
			if (count($Perfil) > 0) {
				...
				echo $json->encode(
					array(
						"v_nombreperfil" =>  $Perfil[0]['v_nombreperfil'],
					)
				);
			}
		} else {
